...
namespace 批量修改图片
{
     ...
     */
        public void changeImageColor(String srcImageFilePath, String resultFilePath, int oldColor, int newColor, double mSimilarity, bool isSingleValue)
        {
            int color1 = oldColor;
            int color2 = newColor;
            ...
            for (int i = 0; i < width; i++)
            {
                for (int j = 0; j < height; j++)
                {
                    ...
                    if (isEqual(r, color1, mSimilarity))
                    {
                        int[] rgb = Hex2RGB(color1);
                        double[] hsl = RGB2HSL(rgb[0], rgb[1], rgb[2]);
                        int[] rgb1 = Hex2RGB(r);
                        double[] hsl1 = RGB2HSL(rgb1[0], rgb1[1], rgb1[2]);
                        int[] rgb2 = Hex2RGB(color2);
     ...
     * @return
     */
        public bool isEqual(int color, int color2, double mSimilarity)
        {
            ...
            double temp =
                Math.Min((Math.Max(hsl[0], hsl2[0]) - Math.Min(hsl[0], hsl2[0])),
                    (Math.Min(hsl[0], hsl2[0]) + 1.0 - Math.Max(hsl[0], hsl2[0])));
